...
def test_update_profile_platform_none_coerces_to_host_platform(tmp_db):
    """update_profile must mirror create_profile's None -> host_platform()
    coercion. Otherwise PUT {"platform": null} persists SQL NULL and
    ProfileResponse.platform: str 500s on the next GET/LIST."""
    p = db.create_profile("t")
    updated = db.update_profile(p["id"], platform=None)
    assert updated["platform"] == db.host_platform()
    assert db.get_profile(p["id"])["platform"] == db.host_platform()
